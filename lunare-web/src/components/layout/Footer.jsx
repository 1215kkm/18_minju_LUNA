    ...
    <footer className="bg-[#6e6a85] text-white/55 px-8 md:px-12 lg:px-28 xl:px-40 pt-14 md:pt-20 pb-10">
      <div className="flex flex-col md:flex-row gap-6 md:gap-0 justify-between mb-8 md:mb-16">
        ...
        <div className="flex-shrink-0">
          ...
          <p className="font-pretendard text-[11px] md:text-[12px] leading-[2] font-light max-w-[200px]">
            Light in its softest form.<br />
            Beauty distilled to its essence.
          </p>
        </div>
          ...
          {Object.entries(FOOTER_LINKS).map(([col, links]) => (
            <div key={col}>
              ...
              <ul className="flex flex-col gap-2.5 md:gap-3.5">
                {links.map((link) => (
                  <li key={link}>
                    <a href="#" className="font-pretendard text-[11px] md:text-[12px] font-light hover:text-white/80 transition-colors duration-300">
                      {link}
                    </a>
                  </li>
                ))}
      ...
      {/* 하단 */}
      <div className="border-t border-white/8 pt-6 md:pt-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 md:gap-4">
        <p className="font-pretendard text-[10px] md:text-[11px] font-light">© 2025 LUNARÉ. All rights reserved.</p>
        <div className="flex flex-wrap gap-x-5 gap-y-2 md:gap-7">
          {['Privacy Policy', 'Terms of Use', 'Cookie Settings'].map((t) => (
            <a key={t} href="#" className="font-pretendard text-[10px] md:text-[11px] font-light hover:text-white/75 transition-colors duration-300">
              {t}
            </a>
          ))}
        </div>
